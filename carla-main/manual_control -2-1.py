...
import os
import sys
sys.path.insert(0, "C:/UnrealEngine/carla/PythonAPI/carla/dist/carla-0.9.15-py3.8-win-amd64.egg")
...
run=pygame.mixer.Sound('run.wav')
...
class World(object):
    ...
    def create_vespa_light(self):
        ...
        spawn_location_3 = carla.Location(x=25.67, y=25.83, z=-1.5) 
    ...
    def create_vespa_light2(self):
        vespa_light_bp = self.world.get_blueprint_library().find('vehicle.vespa.zx125')
        spawn_location_3 = carla.Location(x=-268.41, y=-26.52, z=-1.5) 
        spawn_rotation_3 = carla.Rotation(pitch=0.0, yaw=90.0, roll=0)
        custom_transform_3 = carla.Transform(spawn_location_3, spawn_rotation_3)
    ...
    def create_slow_car(self):
        vehicle_bp_2 = self.world.get_blueprint_library().find('vehicle.audi.tt')
        spawn_location_2 = carla.Location(x=91.07, y=28.50, z=0.2) 
        spawn_rotation_2 = carla.Rotation(pitch=0.0, yaw=180.0, roll=0)
    ...
    def create_stop_car(self):
        ...
        spawn_location_3 = carla.Location(x=-236.7, y=-24.44, z=0.2) 
...
def horn():
    ...
    if event_stopcar==1:
        event_stopcar+=1   
slowcar_state=0 
def control_slowcar(world, state):
    global slowcar_state
    control_2 = carla.VehicleControl()
    # throttle 與 steer 值資料表
    location=world.vehicle2.get_location()
    traffic_light = world.vespa_light.get_traffic_light()
    if location.x<=75 and slowcar_state==0:
        slowcar_state+=1
    if location.x<=52 and slowcar_state==1:
        slowcar_state+=1
    elif traffic_light.get_state() == carla.TrafficLightState.Yellow and slowcar_state==2:
        light_state = carla.VehicleLightState.NONE 
        world.vehicle2.set_light_state(carla.VehicleLightState(light_state))
        slowcar_state+=1
    elif location.x<=17 and slowcar_state==3:
        slowcar_state+=1
        
    if slowcar_state==0:
        control_2.throttle = 0.5
        control_2.steer = 0
    elif slowcar_state==1 or slowcar_state==2:
        control_2.throttle = 0.17
        control_2.steer = 0
    elif slowcar_state==3: 
        control_2.throttle = 1
        control_2.steer = 0  
    elif slowcar_state==4:  
        control_2.throttle = 0
        control_2.steer = 0  
        control_2.brake = 1  
    else:
        world.vehicle2.apply_control(carla.VehicleControl(throttle=0.0, steer=0.0, brake=1.0)) 
        return 1
    if world.vehicle2 is not None:
        world.vehicle2.apply_control(control_2)
    return 0
stopcar_state=0 
def control_stopcar(world, state):
    global stopcar_state
    control_2 = carla.VehicleControl()
    location=world.vehicle3.get_location()
    traffic_light = world.vespa_light2.get_traffic_light()
    # throttle 與 steer 值資料表
    light_state = carla.VehicleLightState.NONE 
    world.vehicle3.set_light_state(carla.VehicleLightState(light_state))
    if stopcar_state==0 and location.x<=-250:
       stopcar_state+=1 
    if stopcar_state==1 and traffic_light.get_state() == carla.TrafficLightState.Green:
       stopcar_state+=1
    ...
    if stopcar_state==0:
        ...
        control_2.steer = 0 
        control_2.brake = 0
    elif stopcar_state==1:
        control_2.throttle = 0
        control_2.steer =0  
        control_2.brake = 1    
    elif stopcar_state==2:
        control_2.throttle = 0.5
        control_2.steer = 0.25 
    else:
...
def game_loop(args):
    ...
    world = None
    writer = imageio.get_writer('output2-1.mp4', fps=50, quality=1, codec='libx264', bitrate='2000k')

    excel_path = "output_data2-1.xlsx"
    workbook = Workbook()
    ...
    walker_state=0
    #設定事件觸發點
    event_location1=carla.Location(x=70.48, y=-14.65, z=0)
    event_location2=carla.Location(x=70.559, y=12.051, z=0)
    event_location3=carla.Location(x=66.396, y=28.922, z=0)
    event_location4=carla.Location(x=33.736, y=25.074, z=0)
    event_location5=carla.Location(x=21.656, y=-0.0405, z=0)
    event_location6=carla.Location(x=-78, y=1.510, z=0)
    event_location7=carla.Location(x=-147.93, y=-0.3846, z=0)
    event_location8=carla.Location(x=-172.2, y=-6.661, z=0)
    event_location9=carla.Location(x=-183.7, y=-23.26, z=0)
    event_location10=carla.Location(x=-220.35, y=-23.26, z=0)
    event_location11=carla.Location(x=-263.2, y=-23.26, z=0)
    event_location12=carla.Location(x=-291.82, y=-19.15, z=0)
    distance_11 = abs(event_location12.x - event_location11.x) + abs(event_location12.y - event_location11.y)
        ...
        while True:
            ...
            if image_number<=4:
                ...
                if start_stop==1:
                    ...
                    if event_slow_car==1:
                        if control_slowcar(world,state_slow_car)==1:
                            event_slow_car=0
                            world.vehicle2.set_location(carla.Location(x=-49.78, y=32.50, z=0.2)) 
                        state_slow_car+=1 
                    if event_stopcar==2:
                        if control_stopcar(world,state_stop_car)==1:
                            event_stopcar=0
                    ...
                    if event_walker==1:
                        ...
                        if current_location.x<=80:
                            channel_run.set_volume(0.1)
                        elif current_location.x<=85:
                            channel_run.set_volume(0.2)
                        elif current_location.x<=90:
                            channel_run.set_volume(0.3)
                        elif current_location.x<=95:
                            channel_run.set_volume(0.4)
                        elif current_location.x<=100:
                            channel_run.set_volume(0.5)
                        elif current_location.x<=105:
                            channel_run.set_volume(0.6)
                        elif current_location.x<=120:
                            channel_run.stop()   
                        control_walker = carla.WalkerControl()
                        control_walker.direction = carla.Vector3D(0, 1, 0) 
                        control_walker.speed = 3
                        world.walker.apply_control(control_walker)  
                        if  collision=='1':
                            event_walker=0
