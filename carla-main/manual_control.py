...
car_start_drive_away=pygame.mixer.Sound('car-start-drive-away.wav')
channel_car_start_drive_away = pygame.mixer.Channel(6)
...
channel_car_horn_beep = pygame.mixer.Channel(9)
...
class World(object):
    ...
    def create_stage1_event1_car(self):
        ...
        #spawn_location_8 = carla.Location(x=160.00, y=-33.667, z=0.2)
    ...
    def create_stage1_event1_car_2(self):
        ...
        #spawn_location_9 = carla.Location(x=192.61, y=41.88, z=0.2)
...
def control_others_car(world, state):
    ...
    if 1 <= state <= len(throttle_steer_pairs):
        ...
        world.vehicle2.set_location(carla.Location(x=200.00, y=-33.667, z=0.2))
        return 1
...
def control_low_case_car(world, state):
    ...
    if 1 <= state <= 100:
        ...
        raw_throttle, steer = throttle_steer_pairs[state - 101]
        control_2.throttle = raw_throttle
        control_2.steer = steer
    else:
        world.vehicle4.set_light_state(carla.VehicleLightState.NONE)
        world.vehicle3.apply_control(carla.VehicleControl(throttle=0.0, steer=0.0, brake=1.0)) 
        world.vehicle4.set_location(carla.Location(x=185.00, y=-33.667, z=5))
...
def control_bad_car(world, state):
    ...
    if 1 <= state <= len(throttle_steer_pairs):
        raw_throttle, steer = throttle_steer_pairs[state - 1]
        control_3.throttle = raw_throttle
        control_3.steer = steer
        light_state = carla.VehicleLightState.Position | carla.VehicleLightState.HighBeam
        ...
        channel_Car_horn_sound.stop()
        return 1
    

    if world.vehicle3 is not None:
        world.vehicle3.apply_control(control_3)
    return 0

def control_event_3_car(world, state):
    control_4 = carla.VehicleControl()
    # throttle 與 steer 值資料表
    throttle_steer_pairs = [
    (0,0) ,   (0,0),
    (0,0) ,   (0,0),
    ...
    if 1 <= state <= len(throttle_steer_pairs):
        raw_throttle, steer = throttle_steer_pairs[state - 1]
        control_4.throttle = raw_throttle
        control_4.steer = steer
        light_state = carla.VehicleLightState.Position | carla.VehicleLightState.HighBeam
        world.vehicle9.set_light_state(carla.VehicleLightState(light_state)) 
    else:
        world.vehicle9.apply_control(carla.VehicleControl(throttle=0.0, steer=0.0, brake=1.0)) 
        world.vehicle9.set_light_state(carla.VehicleLightState.NONE)
        channel_Car_horn_sound.stop()
        return 1
    

    if world.vehicle9 is not None:
        world.vehicle9.apply_control(control_4)
    return 0
volume_states = {
    "driving": {
...
def game_loop(args):
    ...
    world = None
    writer = imageio.get_writer('output.mp4', fps=50, quality=1, codec='libx264', bitrate='2000k')
    ...
    sheet.append([
        ...
        "x","y","z","wrong_way_time",
        "wrong_blinker","collision_times",
        "Sudden_brake","highest_speed","cross_line_times",
        "Honking_the_horn","speedly_time"
    ])
    ...
    event_location6=carla.Location(x=189.54, y=-8.078, z=0)
    event_location6_2=carla.Location(x=165.90, y=-10.664, z=0)
    event_location7=carla.Location(x=162.83, y=36.312, z=0)
    event_location8=carla.Location(x=166.88, y=50.698, z=0)
    event_location9=carla.Location(x=196.982, y=38.454, z=0)
    event_location10=carla.Location(x=81.98, y=32.768, z=0)
    event_location11=carla.Location(x=124.33, y=32.53, z=0)
    current_location_state=0
    try:
        client = carla.Client(args.host, args.port)
        client.set_timeout(2.0)
        display = pygame.display.set_mode((args.width, args.height), pygame.HWSURFACE | pygame.FULLSCREEN )
                ...
                if start_stop==1:
                    ...
                    if (current_time - last_time) >= cycle:
                        ...
                        if player:
                            ...
                            if current_location_state==0:
                            ...
                            elif current_location_state==7 and rotation.yaw>=-238 and rotation.yaw<=-138:
                                dx = current_location.x - event_location6.x
                                dy = current_location.y - event_location6.y  
                                if math.sqrt(dx*dx + dy*dy)<=5 :
                                    current_location_state+=1
                                    take_place_event=1
                                    world.vehicle3.set_location(carla.Location(x=165, y=8.65, z=0.2))
                                    light_state = carla.VehicleLightState.Position | carla.VehicleLightState.HighBeam
                                    world.vehicle2.set_light_state(carla.VehicleLightState(light_state))
                                    channel_car_start.play(car_start_sound)
                                    channel_car_start.set_volume(0.8,0.3)
                                    
                            elif current_location_state==8:
                                dx = current_location.x - event_location6_2.x
                                dy = current_location.y - event_location6_2.y  
                                if math.sqrt(dx*dx + dy*dy)<=3 :
                                    current_location_state+=1
                                    take_place_event_1_2=2
                                    
                            elif current_location_state == 9:
                                dx = current_location.x - event_location7.x
                                dy = current_location.y - event_location7.y
                                distance = math.sqrt(dx * dx + dy * dy)
                                if distance <= 5 and 80 <= rotation.yaw <= 110:                       
                                    current_location_state += 1
                                    world.vehicle8.set_location(carla.Location(x=185.00, y=-33.667, z=0.2))  
                                    world.vehicle9.set_location(carla.Location(x=192.61, y=41.88, z=0.2)) 
                                ...
                                if distance <= 5 and -30 <= rotation.yaw <= 30:                       
                                    ...
                                    channel_slow_moving.set_volume(1)  
                                ...
                                if distance <= 3.5 and -129 <= rotation.yaw <= -59:                       
                                    ...
                                    channel_car_start_drive_away.play(car_start_drive_away)
                                    channel_car_start_drive_away.set_volume(0.8)
                            
                            elif current_location_state == 12:
                                if  current_location.x > 190 and 17<current_location.y <21:                       
                                    current_location_state += 1
                                    print("pass")
                                    pass_level=1
                            ...
                            if take_place_event == 1:
                                state += 1
                                if control_others_car(world, state)==1:
                                    take_place_event = 0
                            ...
                            if take_place_event_3==1:
                                state_4+=1
                                if control_event_3_car(world, state_4)==1:
                                    take_place_event_3 = 0            
                                        
                            if take_place_event_1_2==2:
                                channel_Car_horn_sound.play(Car_horn_sound)
                                take_place_event_1_2=2_2
                                state_2=0           
                                
                            elif take_place_event_1_2==2_2:
                                state_2+=1    
                                if control_bad_car(world, state_2)==1:
                                    take_place_event_1_2=0  
